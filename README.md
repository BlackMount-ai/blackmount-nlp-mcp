...
45 text analysis tools as a FastMCP server. No NLTK. No spaCy. No transformers. One dependency (`mcp[cli]`), 42 KB of NLP code, ready in seconds.
...
| Tool | Description | Try asking |
|------|-------------|------------|
| `flesch_reading_ease` | 0–100 ease score (higher = easier) | "How readable is this article?" |
...
| `smog_grade_index` | SMOG grade (recommended for healthcare text) | "Is this medical document easy to understand?" |
...
| `get_aspect_sentiment` | Sentiment scoped to specific topics | "What's the sentiment around 'pricing' and 'support' in these reviews?" |

### Keyword Extraction (4 tools)

| Tool | Description | Try asking |
|------|-------------|------------|
| `extract_tfidf_keywords` | TF-IDF keyword ranking across a corpus | "What are the most important terms across these documents?" |
...
| `get_normalized_edit_distance` | Edit distance normalized to 0–1 | "How different are these two strings on a 0-1 scale?" |
| `get_longest_common_subsequence` | LCS length between two strings | "What's the longest common subsequence of these?" |
...
| Tool | Description | Try asking |
|------|-------------|------------|
| `detect_text_language` | Identify language from 20 supported languages | "What language is this text written in?" |
| `detect_text_encoding_type` | Detect script type: ASCII, Latin, Cyrillic, CJK, Arabic, etc. | "What script does this text use?" |
...
| `get_text_statistics` | Full document stats: word count, readability, language, reading time | "Give me a complete statistical profile of this text" |
